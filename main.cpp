...
int main()
{
    ...
    Button test_button({ 300.f, 300.f }, { 100.f,100.f }, sf::Color::Red, font, "test_button", sf::Color::Magenta, true);
    ...
    while (window.isOpen())
    {
        while (const std::optional event = window.pollEvent())
        {
            ...
            if (event->is<sf::Event::MouseButtonPressed>() && test_button.check_mouse_overlap(window))
            {
                ...
                {
                    test_button.set_text("Pressed");
                    test_button.setFillColor(sf::Color::Green);
                }
                else
                {
                    test_button.set_text("Not Pressed");
